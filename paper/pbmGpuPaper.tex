...
The PBM algorithm was designed to utilize each core of the GPU till all the GPU 
...
be performed for each time step when a single compartment was single. While, this 
number increased to $1048576$ per time step when the number of compartments was 
increased to $16$. SMs divide these calculations in blocks of 32 threads and send 
it to the cores for calculations which accounts for some overheard time during the 
simulation. This overhead is present for each timestep, which can be compensated 
by the number of calculations running in parallel on the GPU. Figure 
\ref{fig:res_gpu_timings} 

\begin{figure}[h]
\centering
\includegraphics[scale=0.7,trim=110 220 120 220, clip]{gpu_timing_plot.pdf}
\caption{Workflow of the GPU code indicating data transfers and execution timeline of the code.}
