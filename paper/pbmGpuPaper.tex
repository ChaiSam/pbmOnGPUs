...
\textbf{Need to still complete PBM }
...
be used together in various systems, thus decreasing the simulation times further.
...
general computing as well. These GPUs dominantly have a massively parallel architecture with 
hundreds to thousands of computational cores which can thousands of active threads 
simulataneously \citep{keckler2011gpus}. Modern GPU computing can be exploited using 
parallel programming languages such as OpenCL and CUDA. There are various commercial 
applications which are now capable of running their code on the GPU as well. Thus, with 
the growing popularity and lower costs, it is possible to run parallel codes on a desktop PC.
Tradionally, large parallel jobs needed to be run on supercomputers which had thousands of 
cores, but these are require special components making them expensive. With the advent of 
general purpose GPU (GPGPU), there has been a trend to move to them due the costs benefits.

In this work the CUDA toolkit developed by NVIDIA was used to parallelize the PBM. T
