...
The PBM algorithm was structured to utilize each core of the GPU till all the GPU 
are used. Each GPU consisted of several streaming multi-processors(SM) which help 
...
be performed for each time step when a single compartment was single. While, the 
number of calculations increased to over 2 million per time step when the number 
of compartments was increased to $32$. SMs divide these calculations in blocks 
...
and 45 seconds of liquid addition. The algorithm speed was tested by soft scaling 
...
code is directly proportional to the number of compartments present in the problem. 
The time taken by the SMs to distribute the calculations between the cores of GPU remains 
fairly constant, thus the amount of time it takes for the 
The number of calculations double as the number of compartment is 
doubled but still there is a decrease in the time taken which can be 

\begin{figure}[h]
\centering
%\includegraphics[scale=0.7,trim=110 220 120 220, clip]{desktop_gpu_timing.pdf}
\includegraphics[scale=0.7]{desktopgputiming.pdf}
\caption{Workflow of the GPU code indicating data transfers and execution timeline of the code.}
